...
import com.example.android.sunshine.Constant;
...
//        long normalizedUtcStartDay = SunshineDateUtils.normalizeDate(now);
        ...
        for (int i = 0; i < jsonWeatherArray.length(); i++) {
            ...
            // send update to wearable device
            updateTodayWear(context, dateTimeMillis, weatherId, high, low);
        }
    ...
    /**
     * check if this weather for today
     * @param context
     * @param time
     * @param weatherId
     * @param high
     * @param low
     */
    private static void updateTodayWear(Context context, long time, int weatherId, double high, double low) {
        Calendar calToday = Calendar.getInstance();
        Calendar calData = Calendar.getInstance();
        calToday.setTime(new Date());
        calData.setTime(new Date(time));
        boolean sameDay = calToday.get(Calendar.YEAR) == calData.get(Calendar.YEAR) &&
                calToday.get(Calendar.DAY_OF_YEAR) == calData.get(Calendar.DAY_OF_YEAR);
        if (!sameDay)
            return;
        sendUpdateToWear(context, weatherId, high, low);
    }

    /**
     * broadcast to main activty to send data to wearable
     * @param context
     * @param weatherId
     * @param high
     * @param low
     */
    private static void sendUpdateToWear(Context context, int weatherId, double high, double low){
        Intent intent = new Intent(Constant.ACTION_WEARBLE);
